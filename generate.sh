...
# ``solver.export_dir`` fields in configs/solver_definitions/solver_definition_mpc_*.yaml resolve
# under ``examples/acados_{position,trajectory}_mpc/``. The resulting
# ``acados_ocp.json`` files therefore embed correct absolute paths to the
# sibling ``mpc_generated_code/`` C code.
#
# Usage:
#   ./generate.sh [position|trajectory|both]      (default: both)
#
# Requirements:
...
PY_ROOT="${SCRIPT_DIR}/build/python"
...
generate_one() {
  local variant="$1"
  local yaml="${SCRIPT_DIR}/configs/solver_definitions/solver_definition_mpc_${variant}.yaml"
  local export_dir="${SCRIPT_DIR}/examples/acados_${variant}_mpc"
...
echo "Done. Regenerated acados artefacts under examples/acados_*_mpc/mpc_generated_code/."
