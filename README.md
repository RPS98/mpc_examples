...
Comparative quadrotor controller showcase. Four outer-loop controllers
(PID geometric, position MPC, steady-state-aware position MPC, trajectory
MPC) paired against five reference generators (waypoints, jerk-limited,
GCOPTER, dynamic polynomial, polynomial via `mav_trajectory_generation_lib`)
on the same simulator (`mav_simulator`, 500 Hz INDI + 1000 Hz rigid-body).
...
PID is compatible with every generator; the position-class MPCs only
consume the waypoint generator and the trajectory MPC only consumes the
smooth generators.
...
`ssa_position_mpc` is the Steady-State-Aware variant of `mpc_position`
(Limón et al. "MPC for tracking"); the augmented artificial steady state
is solved jointly with the trajectory. See
[github.com/RPS98/ssa_position_mpc](https://github.com/RPS98/ssa_position_mpc)
for the formulation. The solver is consumed in-place from the external repo at
`../thirdparty_libs/ssa_position_mpc/acados_ssa_position_mpc/` and its acados
code is generated by `bash libs/generate_acados.sh ssa_position` (called
automatically from `build.sh`).
...
./scripts/single/ssa_position_mpc_waypoints_cpp.sh
./scripts/single/mpc_trajectory_gcopter_py.sh
./scripts/single/pid_jerk_limited_cpp.sh
# ... one per supported (controller, generator) pair in both languages
```
