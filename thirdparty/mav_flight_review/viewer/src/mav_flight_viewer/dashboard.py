...
"""Comparative dashboard for a full mpc_examples unified run (MCAP backend).

Reads ``<run_dir>/{cpp,py}/*.mcap`` (plus ``<run_dir>/metrics/summary.csv``
when present) and writes ``<run_dir>/plots/dashboard.png`` with the same
layout as the retired ``mav_flight_logger.dashboard`` tool:
...
3. Mean settling time bar chart (from summary.csv).
4. Controller compute-time boxplot per combination.
5. Generator compute-time (update + eval) boxplot per combination.
6. Jerk energy bar chart (from summary.csv).
"""
...
from .flight_frame import load_flight_frame
...
def _collect_logs(run_dir: str) -> Dict[Tuple[str, str, str], dict]:
    """Return dict keyed by (lang, controller, generator) → flight-frame dict."""
    ...
    for lang in ('cpp', 'py'):
        ...
        for fname in sorted(os.listdir(lang_dir)):
            ...
            frame = load_flight_frame(os.path.join(lang_dir, fname))
            if 'time' not in frame.data or len(frame['time']) == 0:
                continue
            logs[(lang, ctrl, gen)] = frame.data
    return logs
...
def build_dashboard(run_dir: str, output_path: Optional[str] = None,
                    show: bool = False) -> str:
    """Produce the comparative dashboard PNG and return its final path."""
    summary = _load_summary_csv(run_dir)
    logs = _collect_logs(run_dir)
    if not logs:
        raise RuntimeError(
            f'No MCAP logs found under {run_dir}/{{cpp,py}}/ — nothing to plot.')
    ...
    if summary:
        ...
        ax_rmse.text(0.5, 0.5,
                     'metrics/summary.csv missing\n'
    ...
    fig.suptitle(
        f'mpc_examples dashboard — {os.path.basename(run_dir.rstrip("/"))}',
        fontsize=14)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Render a comparative dashboard from MCAP logs.')
