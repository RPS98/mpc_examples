...
Python mirror of
``examples_cpp/src/controllers/pid_position_geometric_controller.cpp``.
...
@dataclass
class PidPositionGeometricConfig:
    """Parsed configuration for :class:`PidPositionGeometricController`."""
...
class PidPositionGeometricController(IController):
    """Cascaded PID for position/waypoint tracking (horizon size 1).
    ...
    Requires only ReferenceField.POSITION. Suitable for waypoint generators
    and position MPC.
    """
    ...
    @staticmethod
    def load_config_from_yaml(path: str) -> PidPositionGeometricConfig:
        """Load a :class:`PidPositionGeometricConfig` from a YAML file.
        ...
        if not isinstance(ctrl, dict):
            raise ValueError("pid_position_geometric config: 'controller' must be a mapping.")
        if 'position' not in ctrl:
            raise ValueError("pid_position_geometric config: 'controller.position' is required.")
        cfg.position_pid_params = _parse_pid_params(ctrl['position'], 'controller.position')
        if 'velocity' not in ctrl:
            raise ValueError("pid_position_geometric config: 'controller.velocity' is required.")
        cfg.velocity_pid_params = _parse_pid_params(ctrl['velocity'], 'controller.velocity')

        geo = ctrl.get('geometric')
        if not isinstance(geo, dict):
            raise ValueError("pid_position_geometric config: 'controller.geometric' must be a mapping.")
        if 'mass' not in geo:
            raise ValueError("pid_position_geometric config: 'controller.geometric.mass' is required.")
    ...
    def compute_command(
        ...
        if not references:
            raise ValueError('PidPositionGeometricController: references must not be empty.')
