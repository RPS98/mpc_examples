...
"""Unified Python example: MpcTrajectoryController + GcopterGenerator.

Python mirror of ``examples/examples/mpc_trajectory_gcopter/run_example.cpp``. Consumes the
...
from mpc_examples_adapters.trajectory_generators import GcopterGenerator
...
def main() -> int:
    args = parse_full_args(
        prog='mpc_trajectory_gcopter',
        default_controller_cfg='configs/controllers/config_mpc_trajectory.yaml',
        default_trajectory_cfg='configs/generators/config_gcopter.yaml',
        default_output_file='simulator_logs/mpc_trajectory_gcopter_log.csv',
    )
    ...
    traj_cfg = GcopterGenerator.load_config_from_yaml(args.trajectory_config_path)

    controller = MpcTrajectoryController(controller_cfg)
    traj_gen = GcopterGenerator(traj_cfg)
