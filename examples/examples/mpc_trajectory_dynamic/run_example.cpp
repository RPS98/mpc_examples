 ...
 * @brief Unified example: MpcTrajectoryController + DynamicTrajectoryGenerator.
 *
...
struct Args {
  ...
  std::string controller_config_path = "configs/controllers/config_mpc_trajectory.yaml";
  std::string trajectory_config_path = "configs/generators/config_dynamic.yaml";
  std::string output_file            = "simulator_logs/mpc_trajectory_dynamic_log.csv";
};
...
int main(int argc, char** argv) {
  ...
  const auto controller_cfg =
      adapters::MpcTrajectoryController::loadConfigFromYaml(args.controller_config_path);
  const auto traj_cfg =
      adapters::DynamicTrajectoryGenerator::loadConfigFromYaml(args.trajectory_config_path);

  auto controller = std::make_unique<adapters::MpcTrajectoryController>(controller_cfg);
