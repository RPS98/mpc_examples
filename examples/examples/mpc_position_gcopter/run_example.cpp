 ...
 * @brief Unified example: MpcPositionController + GcopterGenerator.
 *
...
struct Args {
  ...
  std::string trajectory_config_path = "configs/generators/config_gcopter.yaml";
  std::string output_file            = "simulator_logs/mpc_position_gcopter_log.csv";
};
...
int main(int argc, char** argv) {
  ...
  const auto traj_cfg =
      adapters::GcopterGenerator::loadConfigFromYaml(args.trajectory_config_path);

  auto controller = std::make_unique<adapters::MpcPositionController>(controller_cfg);
  auto traj_gen   = std::make_unique<adapters::GcopterGenerator>(traj_cfg);
