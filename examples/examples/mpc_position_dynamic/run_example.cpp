 ...
 * @brief Unified example: MpcPositionController + DynamicTrajectoryGenerator.
 *
...
#include "adapters/controllers/mpc_position_controller.hpp"
...
struct Args {
  ...
  std::string controller_config_path = "configs/controllers/config_mpc.yaml";
  std::string trajectory_config_path = "configs/generators/config_dynamic.yaml";
  std::string output_file            = "simulator_logs/mpc_position_dynamic_log.csv";
};
...
int main(int argc, char** argv) {
  ...
  const auto controller_cfg =
      adapters::MpcPositionController::loadConfigFromYaml(args.controller_config_path);
  const auto traj_cfg =
      adapters::DynamicTrajectoryGenerator::loadConfigFromYaml(args.trajectory_config_path);

  auto controller = std::make_unique<adapters::MpcPositionController>(controller_cfg);
  auto traj_gen   = std::make_unique<adapters::DynamicTrajectoryGenerator>(traj_cfg);
